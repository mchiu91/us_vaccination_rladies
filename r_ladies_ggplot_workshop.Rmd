...
# read reference map into R and assign it to an object
us_map <- map_data("state")
head(us_map) # peek at the reference map data
```
...
covid_map_1 <- 
  ...
  labs(title = "State-by-state Vaccination Across the United States",
       caption = "Data from the Centers for Disease Control and Prevention",
...
## Find other state data

### There are other places to find state-level data that might be interesting. The US census: https://www.census.gov/data/datasets.html

### Base R even has some built-in state data: https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/state

# need to take the state.x77 matrix and make it into
# something that state_choropleth can use

# Could ask participants to take the state.x77 data and use either/both 
...
state_choropleth(state_murder_plot,
                 title = "murder and non-negligent manslaughter rate",
                 legend = "per 100,000 population (1976)",
                 num_colors = 1)
```