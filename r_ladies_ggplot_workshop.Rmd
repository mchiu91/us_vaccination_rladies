...
Read in our US state data and assign it to an object **us_map**, which has *long* and *lat* (longitude and latitude) variables used in outlining our US state map.
...
Next, we will tweak our **today_covid** dataframe to fit the required format for state_choroplethr, then merge by "region" to make our final 'master' data **today_covid_map** that will be used in both ggplot2 and choroplethr map visuals.
...
```{r plot-1, warning = FALSE, results = 'asis'}
